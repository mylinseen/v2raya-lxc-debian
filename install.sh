...
install_v2raya() {
    ...
    for i in "${!MIRROR_URLS[@]}"; do
        ...
        if wget -q --timeout=30 --tries=2 --retry-connrefused -O v2raya.zip "$url"; then
    ...
    if [ "$download_success" = false ]; then
        ...
        log_info "3. 访问 https://github.com/v2rayA/v2rayA/releases 查看最新版本"
        exit 1
    fi
